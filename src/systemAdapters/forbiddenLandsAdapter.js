...
export class ForbiddenLandsAdapter extends BaseAdapter {
    constructor() { super('forbidden-lands') }

    getActorHp(actorOrToken) {
        const a = (actorOrToken?.actor) ? actorOrToken.actor : actorOrToken
        return getProperty(a, 'system.attributes.hp.value') ?? getProperty(a, 'system.hp') ?? getProperty(a, 'system.health.value') ?? super.getActorHp(a)
    }

    getActorMaxHp(actorOrToken) { return getProperty(actorOrToken, 'system.attributes.hp.max') ?? super.getActorMaxHp(actorOrToken) }

    getActorAc(actor) { return getProperty(actor, 'system.defenses.ac') ?? super.getActorAc(actor) }

    getAttackItems(actor) { return (actor?.items || []).filter(i => i.type === 'weapon' || i.system?.damage) }
...
export default ForbiddenLandsAdapter
