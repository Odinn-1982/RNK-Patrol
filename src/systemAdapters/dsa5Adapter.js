...
export class Dsa5Adapter extends BaseAdapter {
    constructor() { super('dsa5') }

    getActorHp(actorOrToken) {
        const a = (actorOrToken?.actor) ? actorOrToken.actor : actorOrToken
        return getProperty(a, 'system.status.wounds.value') ?? getProperty(a, 'system.status.hp.value') ?? super.getActorHp(a)
    }

    getActorMaxHp(actorOrToken) { return getProperty(actorOrToken, 'system.status.wounds.max') ?? super.getActorMaxHp(actorOrToken) }

    getActorAc(actor) { return getProperty(actor, 'system.defense') ?? super.getActorAc(actor) }
