...
export class UnhallowedMetropolisAdapter extends BaseAdapter {
    constructor() { super('unhallowed-metropolis') }
    ...
    getActorAc(actor) { return getProperty(actor, 'system.attributes.ac.value') ?? super.getActorAc(actor) }
    getAttackItems(actor) { return (actor?.items || []).filter(i => i.type === 'weapon' || i.type === 'attack') }

    async estimateBestAttackForToken(token) {
        const actor = token?.actor
        if (!actor) return super.estimateBestAttackForToken(token)
...
export default UnhallowedMetropolisAdapter
