...
export class Pf1Adapter extends BaseAdapter {
    constructor() { super('pf1') }

    getActorHp(actorOrToken) {
        const a = (actorOrToken?.actor) ? actorOrToken.actor : actorOrToken
        return getProperty(a, 'system.attributes.hp.value') ?? getProperty(a, 'system.hp') ?? super.getActorHp(a)
    }

    getActorMaxHp(actorOrToken) { return getProperty(actorOrToken, 'system.attributes.hp.max') ?? super.getActorMaxHp(actorOrToken) }

    getActorAc(actor) { return getProperty(actor, 'system.attributes.ac.value') ?? getProperty(actor, 'system.attributes.ac') ?? super.getActorAc(actor) }

    getAttackItems(actor) { return (actor?.items || []).filter(i => i.type === 'weapon' || i.type === 'melee' || i.type === 'ranged' || i.type === 'item') }
    ...
    async estimateBestAttackForToken(token) {
        ...
        for (const w of weapons) {
            ...
            const atk = Number(getProperty(w, 'system.attack') || getProperty(w, 'system.attackBonus') || 0)
