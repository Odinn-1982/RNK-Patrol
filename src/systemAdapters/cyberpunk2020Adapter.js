...
export class Cyberpunk2020Adapter extends BaseAdapter {
    constructor() { super('cyberpunk-2020') }

    getActorHp(actorOrToken) {
        const a = (actorOrToken?.actor) ? actorOrToken.actor : actorOrToken
        // Cyberpunk may use system.stats.hp or system.attributes.hp
        return getProperty(a, 'system.stats.hp') ?? getProperty(a, 'system.stats.body') ?? getProperty(a, 'system.hp') ?? super.getActorHp(a)
    }

    getActorMaxHp(actorOrToken) { return this.getActorHp(actorOrToken) ?? super.getActorMaxHp(actorOrToken) }

    getActorAc(actor) { return getProperty(actor, 'system.attributes.evasion') ?? getProperty(actor, 'system.defenses.ac') ?? super.getActorAc(actor) }

    getAttackItems(actor) { return (actor?.items || []).filter(i => i.type === 'weapon' || i.system?.weapon) }
    ...
    async estimateBestAttackForToken(token) {
        ...
        for (const w of weapons) {
            ...
            const atk = Number(getProperty(w, 'system.attack') || 0)
...
export default Cyberpunk2020Adapter
