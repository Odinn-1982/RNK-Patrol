...
export class BladesInTheDarkAdapter extends BaseAdapter {
    constructor() { super('blades-in-the-dark') }

    getActorHp(actorOrToken) {
        const a = (actorOrToken?.actor) ? actorOrToken.actor : actorOrToken
        return a?.system?.health?.value ?? a?.system?.hp ?? a?.system?.attributes?.hp?.value ?? super.getActorHp(a)
    }

    getActorMaxHp(actorOrToken) {
        const a = (actorOrToken?.actor) ? actorOrToken.actor : actorOrToken
        return a?.system?.health?.max ?? a?.system?.hp?.max ?? a?.system?.attributes?.hp?.max ?? super.getActorMaxHp(a)
    }

    async estimateBestAttackForToken(token) {
        // Blades uses 'action' or 'ability', fallback to items with damage
        const actor = token?.actor
        if (!actor) return null
        const weapons = actor.items?.filter(i => i.type === 'action' || i.system?.damage) || []
        let best = null
...
export default BladesInTheDarkAdapter
